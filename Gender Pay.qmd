...
#gss_wages <- filter(gss_wages, !is.na(gender))
...
```{r}
# Create an ECDF plot
ggplot(wage_data_filtered, aes(x = realrinc, color = wrkstat)) +
  stat_ecdf(geom = "step", size = 1.5) +
  labs(title = "ECDF of Income Across Work Status",
       x = "Income",
...
ggplot(wage_data_filtered, aes(sample = realrinc, color = wrkstat)) +
  stat_qq() +
  labs(title = "Quantile-Quantile Plot of Income Across Work Status",
...
```{r}
# Create a table
marital_child_table <- table(wage_data_filtered$maritalcat, wage_data_filtered$childs)

# Convert the table to a data frame
marital_child_df <- as.data.frame(as.table(marital_child_table))

# Rename the columns for better clarity
colnames(marital_child_df) <- c("Marital Status", "Number of Children", "Count")

# Print the resulting data frame
print(marital_child_df)

# Create a grouped bar plot
ggplot(marital_child_df, aes(x = interaction(`Marital Status`, `Number of Children`), y = Count, fill = `Number of Children`)) +
  geom_bar(stat = "identity", position = "dodge", color = "white") +
  labs(title = "Relationship Between Marital Status, Number of Children, and Average Income",
       x = "Marital Status and Number of Children",
       y = "Count",
